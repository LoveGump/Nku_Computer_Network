...
int ChatWindow::run(HINSTANCE hInst, int nCmdShow) {
    const wchar_t CLASS_NAME[] = L"ChatClientWin32OOP";  // 窗口类名
    WNDCLASSW wc{};
    ...
    HWND hwnd = CreateWindowExW(
        0, CLASS_NAME, L"Chat Client", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT,
        CW_USEDEFAULT, 640, 420, nullptr, nullptr, hInst, this);

    // 显示窗口
    ShowWindow(hwnd, nCmdShow);

    // 消息循环，如果GetMessage返回0则退出
    MSG msg;
    while (GetMessageW(&msg, nullptr, 0, 0) > 0) {
        TranslateMessage(&msg);  // 把键盘消息翻译生成 WM_CHAR 等字符消息
 ...
 * @param lParam 消息参数
 */
LRESULT CALLBACK ChatWindow::WndProcThunk(HWND hwnd, UINT msg, WPARAM wParam,
                                          LPARAM lParam) {
    ...
    if (msg == WM_NCCREATE) {
        ...
        // 否则从窗口数据中获取实例指针
        self = reinterpret_cast<ChatWindow*>(
...
LRESULT ChatWindow::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_CREATE: {
            ...
            // 连接参数输入框
            // 地址WS_CHILD 子窗口 | WS_VISIBLE 可见 | WS_BORDER 有边框 |
            // ES_AUTOHSCROLL 自动水平滚动
            hAddr_ = CreateWindowW(
                L"EDIT", L"127.0.0.1",
                WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL, 10, 10, 180,
                24, hwnd, (HMENU)IDC_ADDR, GetModuleHandleW(nullptr), nullptr);
            // 端口
            hPort_ = CreateWindowW(
                L"EDIT", L"5000",
                WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL, 200, 10, 60,
                24, hwnd, (HMENU)IDC_PORT, GetModuleHandleW(nullptr), nullptr);
            // 昵称
            hNick_ = CreateWindowW(
                L"EDIT", L"User",
                WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL, 270, 10,
                120, 24, hwnd, (HMENU)IDC_NICK, GetModuleHandleW(nullptr),
                nullptr);
            // 连接按钮
            hConnect_ = CreateWindowW(L"BUTTON", L"连接", WS_CHILD | WS_VISIBLE,
                                      400, 10, 70, 24, hwnd, (HMENU)IDC_CONNECT,
            ...
            hChat_ =
                CreateWindowW(L"EDIT", L"",
                              ...
                              10, 44, 460, 260, hwnd, (HMENU)IDC_CHATLOG,
                              GetModuleHandleW(nullptr), nullptr);
            // 输入框
            hInput_ = CreateWindowW(L"EDIT", L"",
                                    WS_CHILD | WS_VISIBLE | WS_BORDER |
                                        ES_AUTOHSCROLL | ES_WANTRETURN,
                                    10, 310, 380, 24, hwnd, (HMENU)IDC_INPUT,
                                    GetModuleHandleW(nullptr), nullptr);
            // 发送按钮
            hSend_ = CreateWindowW(L"BUTTON", L"发送", WS_CHILD | WS_VISIBLE,
                                   400, 310, 70, 24, hwnd, (HMENU)IDC_SEND,
                                   GetModuleHandleW(nullptr), nullptr);
            ...
            oldInputProc_ = (WNDPROC)SetWindowLongPtrW(
                hInput_, GWLP_WNDPROC, (LONG_PTR)ChatWindow::InputProcThunk);
            SetPropW(hInput_, L"ChatWindowThis", this);

            // 设置回调函数 ，没有有变化的时候在这里运行
            ...
            client_.setStateCallback([this](bool connected) {
                PostMessageW(hwnd_, WM_CONN_STATE, (WPARAM)(connected ? 1 : 0),
                             0);
            });
            break;
        }
 ...
 * @param lParam 消息参数
 */
LRESULT CALLBACK ChatWindow::InputProcThunk(HWND hwnd, UINT msg, WPARAM wParam,
                                            LPARAM lParam) {
    // 获取 ChatWindow 实例指针
    ChatWindow* self =
        reinterpret_cast<ChatWindow*>(GetPropW(hwnd, L"ChatWindowThis"));
    if (!self) return DefWindowProcW(hwnd, msg, wParam, lParam);
...
void ChatWindow::appendText(const std::wstring& text) {
    ...
    SendMessageW(hChat_, EM_SETSEL, (WPARAM)len,
                 (LPARAM)len);  // 设置选择范围到文本末尾
    SendMessageW(hChat_, EM_REPLACESEL, FALSE,
                 (LPARAM)text.c_str());          // 插入新文本
...
void ChatWindow::updateUiForConnected(bool connected) {
    ...
    // 连接参数在连接后禁用，断开后启用
    EnableWindow(hAddr_, !connected);  // 地址
    EnableWindow(hPort_, !connected);  // 端口
    EnableWindow(hNick_, !connected);  // 名称
    // 发送与输入框仅在连接后启用
    EnableWindow(hSend_, connected);   // 发送
    EnableWindow(hInput_, connected);  // 输入框
}
