...
namespace rtp {
	...
	class TransferStats {
	   public:
		...
		uint32_t get_fast_retransmit_count() const { return fast_retransmit_count_; }
		// 获取开始时间
		uint64_t get_start_time() const { return start_time_; }
		// 获取结束时间
		uint64_t get_end_time() const { return end_time_; }

		// 计算传输耗时（秒）从start_time到end_time的时间差
		double get_elapsed_seconds() const;

		// 计算吞吐率（MiB/s）吞吐率 = 字节数 / 耗时 / 1024 / 1024
		double get_throughput(size_t bytes) const;

		// 计算丢包率（百分比）：丢包率 = 重传次数 / 总段数 * 100%
		double get_loss_rate(uint32_t total_segments) const;

		// 打印发送端统计信息
		void print_sender_stats(size_t file_size, uint32_t total_segments, double cwnd, double ssthresh) const;

		// 打印接收端统计信息
		void print_receiver_stats(size_t bytes_received, uint32_t total_packets, uint32_t out_of_order,
